...
async function main() {
  console.log("Seeding restaurant data with images and addons for all tenants...")

  const tenants = await prisma.tenant.findMany({ select: { id: true, name: true } })
  const tenantIds = tenants.map(t => t.id)
  if (!tenantIds.includes("plat_default")) tenantIds.push("plat_default")

  for (const tenantId of tenantIds) {
    console.log(`Processing tenant: ${tenantId}`)

    // 1. Seed Tables
    const existingTablesCount = await prisma.restaurantTable.count({ where: { tenantId } })
    if (existingTablesCount === 0) {
      ...
      console.log(`  Seeded ${demoTables.length} tables for ${tenantId}`)
    }

    // 2. Seed Categories & Dishes with Addons & Images
    ...
    console.log(`  Seeded menu categories and dishes with images & addons for ${tenantId}`)
  }

  console.log("Restaurant seeding with images completed successfully!")
}
